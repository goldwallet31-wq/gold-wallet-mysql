...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      console.log('🔍 Login: فحص الجلسة النشطة...')
      const { data: { session } } = await supabase.auth.getSession()
      if (session) {
        console.log("✅ Login: جلسة نشطة موجودة، إعادة التوجيه إلى الصفحة الرئيسية...")
        console.log("👤 Login: معرف المستخدم:", session.user.id)
        console.log("📧 Login: البريد الإلكتروني:", session.user.email)
        router.replace('/')
      } else {
        console.log('⚠️ Login: لا توجد جلسة نشطة')
      }
    }
    checkSession()
  }, [router])
